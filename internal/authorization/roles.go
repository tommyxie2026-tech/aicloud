...
func BuiltinRBAC() *RBAC {
	return &RBAC{Version: "builtin-rbac-v1"}
}
...
func (r *RBAC) Evaluate(req Request) Decision {
	...
	if r == nil {
		return Decision{Layer: "rbac", PolicyVersion: version, Reason: "role evaluator is not configured"}
	}
	for _, value := range req.Principal.Roles {
		role := strings.ToLower(strings.TrimSpace(value))
		if roleAllows(role, req.Action) {
			return Decision{Allowed: true, Layer: "rbac", PolicyVersion: version, MatchedRole: role, Reason: "role grants action"}
		}
	}
...
func roleAllows(role string, action Action) bool {
	...
	if isReadAction(action) {
		return role == RoleProjectAdmin || role == RoleDeveloper || role == RoleOperator || role == RoleReviewer || role == RoleViewer
	}
	switch action {
