...
func TestDeveloperCanCreateTaskInOwnProject(t *testing.T) {
	...
	if err != nil || !decision.Allowed || decision.MatchedRole != RoleDeveloper { t.Fatalf("decision=%+v err=%v", decision, err) }
}
...
func TestViewerCannotMutateTask(t *testing.T) {
	...
	if decision.Allowed || decision.Layer != "rbac" { t.Fatalf("unexpected decision: %+v", decision) }
}
...
func TestTenantAdminCannotCrossProjectAttributeBoundary(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if decision.Allowed || decision.Layer != "abac" { t.Fatalf("unexpected decision: %+v", decision) }
}
...
func TestNoRoleFailsClosed(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if decision.Allowed { t.Fatalf("unexpected allow: %+v", decision) }
}
