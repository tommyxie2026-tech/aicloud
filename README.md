...
## 10. v0.1 Skeleton Implementation

The Skeleton PR adds a runnable modular-monolith baseline for the Developer AI Cloud path:
...
The v0.1 API exposes `/healthz`, `/readyz`, `GET/POST /api/v1/models`, and `GET/POST /api/v1/tasks` plus task lookup by ID. Runtime persistence is in-memory for fast startup; PostgreSQL and Redis are available through `docker compose up -d`, and the migration is the contract for the next persistence adapter.

The implementation intentionally keeps one deployable application and separates internal packages by domain. Temporal, provider-backed routing, OPA, OpenTelemetry, and stronger sandbox runtimes remain integration seams for subsequent PRs.